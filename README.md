...
- ##### The connection between major pollutants(PM2.5 and NO2) and the Air Quality Index, correlation between the pollutants and the relation to Covid-19 are the focus areas of the Exploratory Data Analysis. The studies have suggested that the Exposure to indoor and outddor pollution can cause adverse health effects and increase in non communicable diseases. Our Aim is twofold, first to understand the relation between the pollutants and second is to understand the relation between the pollutants and the Covid-19. We collected Data from the EPA (To assess the relationship between air pollutant (PM2.5 and NO2) concentrations and COVID-19 fatality in District of Columbia state,we employ daily data at the city level)and We conducted a statistical analysis that confirms possible correlations among the number of daily cases and several factors related to the air quality(Andrea et al.)
### Linear Regression
